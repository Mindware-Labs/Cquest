...
/* La marca de éxito: un círculo y un check que se DIBUJAN, no que aparecen.
   Compartida por login/ y reset-password/ — vivía sólo en login/ como
   LoginSuccessMark, se generalizó (título y leyenda por prop) cuando el
   segundo flujo necesitó exactamente el mismo gesto de cierre. Misma lógica
   que ya siguieron auth.css y authFont.ts: mover en vez de duplicar.
   ...
   porta significado y cae bajo WCAG 1.4.11 igual que antes.
   ...
   el mismo resplandor que ya vive arriba de la tarjeta (auth.css).
   ...
   vacío en una composición, y de paso es lo que anuncia el éxito: la marca
   dibujada no la anuncia nadie, pero este texto sí. */
...
/* Alto al que se encoge el marco que envuelve esta marca — el suficiente
   para el icono (56px + su halo) más el titular y la leyenda, con aire
   alrededor. Vive acá y no en cada pantalla que la usa porque está atada al
   tamaño FIJO de este contenido, no a la pantalla que lo dispara: login y
   reset-password comparten el mismo número por la misma razón que comparten
   el componente. */
export const SUCCESS_FRAME_HEIGHT = 220;
  ...
  return (
    ...
    >
      {/* `iconRef` es lo que hace el pulso final al cerrar el check — sólo el
          ...
          "rebota". Decorativo: lo que dice "esto salió bien" ahora es el
          texto de abajo, no el dibujo. */}
