...
// `TemplateItem` cruza al cliente sin el árbol de bloques: la miniatura se renderiza acá y baja como HTML.
...
// El origen es un recorte de la lista, no una sección aparte: misma gramática que la barra de Artículos.
const FILTERS = [
...
// Mismos rótulos que usa la miniatura; viven acá porque la búsqueda también los mira (buscar sólo por nombre no alcanza).
...
// Conserva todo lo que ya estaba puesto: sin esto, tocar un filtro borra la búsqueda y viceversa.
...
function normalize(value: string) {
  // Sin acentos y en minúscula: "Articulo" tiene que encontrar "Artículo".
  return value
    .normalize("NFD")
    .replace(/[̀-ͯ]/g, "")
    .toLowerCase();
...
export default async function AdminTemplatesPage({
  ...
  // El recorte vive en la URL y no en estado de cliente: se puede compartir, recargar y volver atrás.
  ...
  // `choiceKey` usa el mismo formato que lib/templateChoices.ts (lo que el editor lee de `?plantilla=`); si se separan, «Usar» abre el editor en blanco sin avisar.
  ...
  const team: Entry[] = saved.map((template) => {
    // Contador de bloques sale del contenido validado: una plantilla que ya no encaja se muestra con 0 en vez de romper la pantalla, y sin choiceKey para no ofrecer un enlace roto.
  ...
  const visible = [...matched].sort(({ item: a }, { item: b }) => {
    ...
    if (sort === "recientes") {
      // Las del sistema no tienen fecha: se ordenan después de las del equipo, ya que "Más recientes" habla de lo que el equipo guardó.
  ...
  return (
    // No dice "Nueva plantilla": acá una plantilla no se crea, se guarda desde el editor con «Guardar como plantilla».
    <ModulePage title="Plantillas" description="Estructuras de bloques reutilizables">
      <div className="cq-enter">
        ...
        {/* Filtro, búsqueda, orden y la acción primaria en una sola barra: el botón estaba suelto arriba sin nada a lo que pertenecer. */}
        <div className="cq-table-toolbar">
          <nav aria-label="Filtrar por origen" className="flex flex-wrap items-center gap-1">
            {FILTERS.map((filter) => (
              <Link
                key={filter.key}
                // Cambiar de pestaña conserva búsqueda y orden: son preferencias de lectura, no parte del recorte.
          ...
          {/* method GET y no un input controlado: funciona sin JS, la URL se puede compartir y el historial se comporta normal. */}
          <form method="get" action="/admin/templates" role="search" className="flex items-center gap-2">
            ...
            {/* Limpiar aparece sólo con algo escrito: junto a un campo vacío sería un control que no hace nada. */}
            {term && (
              <Link
        ...
        {visible.length === 0 ? (
          // Dos vacíos distintos: "sin resultados" obligaría a adivinar si el problema es la búsqueda o que no hay nada.
          term ? (
            <EmptyState
          ...
          // Grilla fluida por ancho de tarjeta y no por dispositivo: a 240px la miniatura deja de leerse como página.
          <ul className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
            {visible.map(({ item, blocks }, index) => (
              <TemplateCard
                ...
                index={index}
                template={item}
                // Miniatura renderizada acá en servidor: si la tarjeta (de cliente) importara BlockRenderer, se llevaría los renderers de bloque al bundle del navegador.
